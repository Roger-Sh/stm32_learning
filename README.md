...
- win<--->USB<--->Fire DAP仿真器<--->JTAG排线<--->开发板（开发板本身需要上电）
    ...
    ![image-20240731225501234](./README.assets/image-20240731225501234.png)
  ...
  - Settings: 
    - Debug:
      ...
      - 注意
        ...
        - 普通 DAP 要试 Debug 时，Reset 选择 HW RESET 或者 VECTRESTET 这样在 Debug 时可以正确控制复位
...
![image-20240708132104775](./README.assets/image-20240708132104775.png)
...
<img src="./README.assets/image-20240708145223993.png" alt="image-20240708145223993" style="zoom:50%;" /><img src="./README.assets/image-20240708145319007.png" alt="image-20240708145319007" style="zoom: 25%;" />





<img src="./README.assets/image-20240708145559166.png" alt="image-20240708145559166" style="zoom:50%;" />
...
![image-20240708151823899](README.assets/image-20240708151823899.png)
...
-   储存器映射分布![image-20240708152358468](./README.assets/image-20240708152358468.png)

    -   Block0-Flash

        ![image-20240708152842511](./README.assets/image-20240708152842511.png)

    -   Block1-SRAM

        ![image-20240708152858045](./README.assets/image-20240708152858045.png)

    -   Block2-片上外设

        ![image-20240708152912307](./README.assets/image-20240708152912307.png)
...
-   为什么叫寄存器
    ...
    -   "寄存器"这个术语在计算机科学和工程领域中是一个专有名词，它源自英文单词 "register"。

    -   在计算机科学中，“寄存器”的“寄”字，可以理解为“暂时存放”或“存储”的意思。这里的“寄”并不是指长期的存储，而是指快速、临时的存储。在中文里，“寄”字有“委托”或“托付”的含义，例如“寄存”通常指将物品暂时存放在某处。

        将这个概念应用到计算机的寄存器上，可以这样理解：

        -   **快速访问**：寄存器能够提供比主存储器（RAM）更快的数据访问速度，因此CPU可以将需要频繁访问的数据“寄存”在寄存器中。
        -   **临时存储**：寄存器通常用于存储CPU当前正在处理的数据或指令，这些数据或指令在处理完毕后可能就会被新的数据或指令替换，因此是一种“临时”的存储。
        -   **操作方便**：CPU内部的算术逻辑单元（ALU）可以直接对寄存器中的数据进行操作，这种操作的便捷性类似于将物品“寄存”在便于取用的地方。
        -   **功能特定**：不同的寄存器可能用于存储不同类型的数据或信息，例如程序计数器（PC）寄存器“寄存”当前正在执行的指令地址，而栈指针（SP）寄存器则“寄存”当前栈顶的位置。

        因此，“寄存器”中的“寄”字，强调的是这种存储单元的临时性和快速访问的特点。
...
-   片上外设区分为三条总线
    ...
    ![image-20240708155038705](./README.assets/image-20240708155038705.png "image-20240708155038705")

-   外设基地址：

    -   总线上挂载着各种外设，这些外设也有自己的地址范围

    ![image-20240708155134523](./README.assets/image-20240708155134523.png)
...
-   外设寄存器
    ...
    -   例：GPIOB 的寄存器
        ...
        ![image-20240708155304427](./README.assets/image-20240708155304427.png)

    -   例：GPIO 端口置位/复位寄存器

        ![image-20240708155610456](./README.assets/image-20240708155610456.png)
